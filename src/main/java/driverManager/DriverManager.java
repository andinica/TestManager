...
public class DriverManager {
  ...
  private String appiumPropertiesFilePath =
      "src/main/resources/properties/driverCapabilities.properties";
  private String driverPropertiesFilePath =
      "src/main/resources/properties/driverCapabilities.properties";
  private String webDriverPropertiesFilePath =
      "src/main/resources/properties/driverCapabilities.properties";
  ...
  private void installPackage(String packageName, String version) {
    String command =
        "npm install -g " + packageName + (version.equals("latest") ? "" : "@" + version);
  ...
  public void setupAppium() {
    ...
    boolean withDeviceFarm =
        Boolean.parseBoolean(appiumProperties.getProperty("appium.withDeviceFarm", "false"));
    ...
    if ("uiautomator2".equalsIgnoreCase(automationName)
        && !isPackageInstalled("appium-uiautomator2-driver")) {
      installPackage("appium-uiautomator2-driver", "");
    }

    if ("xcuitest".equalsIgnoreCase(automationName)
        && !isPackageInstalled("appium-xcuitest-driver")) {
      ...
      String command =
          "webdriver-manager update --"
              ...
              + (version.equals("latest")
                  ? " --versions." + driverName + " " + browserVersion
                  : " --versions." + driverName + " " + version);
      executeCommand(
          command,
          "Installed "
              + driverName
              + " version: "
              + (version.equals("latest") ? browserVersion : version));
    } else {
      throw new InvalidPropertiesFormatException(
          String.format("Driver %s is not supported!", driverName));
    }
  }
  ...
  private String getInstalledBrowserVersion(String driverName) throws IOException {
    ...
    java.io.BufferedReader reader =
        new java.io.BufferedReader(
            new java.io.InputStreamReader(new ProcessBuilder(command).start().getInputStream()));
